...
def draw_outline(img, mask, clr=(100, 240, 39), thickness=2):
    ...
    for cnt in contours:
        outline_img = cv2.drawContours(
            outline_img, [cnt], 0, color=int_color, thickness=thickness
        )
...
def draw_matches(src_img, kp1_xy, dst_img, kp2_xy, rad=3, alignment="horizontal"):
    ...
    for i in range(n_pt):
        ...
        line_rc = np.array(
            draw.line_aa(
                *np.round(xy1[::-1]).astype(int), *np.round(xy2[::-1]).astype(int)
            )
        )
...
def get_grid(shape, grid_spacing, thickness=1):
    ...
    for k in range(thickness):
        for i in np.arange(
            grid_spacing - thickness, shape[0] + thickness, grid_spacing
        ):
    ...
    return np.array(all_rows, dtype=uniTupleDtype), np.array(
        all_cols, dtype=uniTupleDtype
    )
...
def rgb_triangle_cmap():
    ...
    tri_edges_x = np.hstack(
        [np.linspace(min_v, max_v, n * 2), np.linspace(max_v - 1 / n, min_v + 1 / n, n)]
    )
    tri_edges_y = np.hstack(
        [
            np.linspace(min_v, max_v, n),
            np.linspace(max_v - 1 / n, min_v, n),
            np.repeat(min_v, n),
        ]
    )
    ...
    bary_xy = np.array(
        [
            np.linalg.inv(T) @ (tri_edges_xy[i] - np.array([1.0, 0.0]))
            for i in range(len(tri_edges_xy))
        ]
    )
...
def tint_grey(grey_img, tint_rgb, outspace="srgb"):
    """
    outspace : "srgb" or "lab"
    """
    if max(tint_rgb) <= 1:
        tint_rgb_255 = (255 * tint_rgb).astype(np.uint8)
    else:
        tint_rgb_255 = tint_rgb

    vips_tint_rgb = warp_tools.numpy2vips(np.array([[tint_rgb_255]]))
    vips_tint_lab = vips_tint_rgb.colourspace(pyvips.enums.Interpretation.LAB)
    tint_lab = warp_tools.vips2numpy(vips_tint_lab)[0][0]
    is_np = isinstance(grey_img, np.ndarray)
    if is_np:
        g = warp_tools.numpy2vips(grey_img)
    else:
        g = grey_img
    ...
    a = pyvips.Image.black(g.width, g.height, bands=1) + tint_lab[1]
    b = pyvips.Image.black(g.width, g.height, bands=1) + tint_lab[2]

    vips_lab = l.bandjoin([a, b])
    vips_lab = vips_lab.copy(interpretation=pyvips.enums.Interpretation.LAB)
    vips_lab = (g == 0).ifthenelse(0, vips_lab)

    if outspace == "lab":
        out_img = vips_lab
    else:
        out_img = vips_lab.colourspace(pyvips.enums.Interpretation.SRGB)

    if is_np:
        out_img = warp_tools.vips2numpy(out_img)

    return out_img


def create_overlap_img(img_list, cmap=jzazbz_cmap(), blending="weighted"):
    """
    img_list : list
        list of single channel images to create overlap from

    blending : str
        "light" uses libvips to blend the image with "LIGHTEN" as the blend mode
        "weighted" weighs each image by its contribution to the total intensity at each pixel
    """
    n_imgs = len(img_list)
    ...
    if is_np:
        is_rgb = (
            img_list[0].dtype == np.uint8
            and img_list[0].ndim == 3
            and img_list[0].shape[2] == 3
        )
        if is_rgb:
        ...
        if is_rgb:
            grey_img_list = [
                x.colourspace(pyvips.enums.Interpretation.B_W) if x.bands == 3 else x
                for x in img_list
            ]
    ...
    if blending == "light":
        if is_np:
            tinted_img_list = [
                tint_grey(warp_tools.numpy2vips(grey_img_list[i]), color_list[i])
                for i in range(n_imgs)
            ]
        else:
            tinted_img_list = [
                tint_grey(grey_img_list[i], color_list[i]) for i in range(n_imgs)
            ]

        base = tinted_img_list[0]
        blended_vips_rgb = base.composite(
            other=tinted_img_list[1:],
            mode=pyvips.enums.BlendMode.LIGHTEN,
            compositing_space=pyvips.enums.Interpretation.SRGB,
        )
        ...
        if is_np:
            ...
            sum_img = (
                pyvips.Image.black(grey_img_list[0].width, grey_img_list[0].height)
                + eps
            )
            blended_img = pyvips.Image.black(
                grey_img_list[0].width, grey_img_list[0].height, bands=3
            )
        ...
        vips_lab_color_list = [
            warp_tools.numpy2vips(np.array([[255 * clr]])).colourspace(
                pyvips.enums.Interpretation.LAB
            )
        ...
        for i in range(len(grey_img_list)):
            ...
            if is_np:
                ...
                blended_img += lab_clr * np.dstack(
                    [grey_img_list[i] / max_v * weight] * 3
                )
            else:
        ...
        if is_np:
            ...
            blended_vips = blended_img

        blended_vips_lab = blended_vips.copy(
            interpretation=pyvips.enums.Interpretation.LAB
        )
        blended_vips_rgb = blended_vips_lab.colourspace(
            pyvips.enums.Interpretation.SRGB
        )[0:3]
    if is_np:
...
def save_overlap_full_rez(registrar, dst_f):
    warped_slide_list = [
        registrar.get_slide(f)
        .warp_slide(level=0)
        .colourspace(pyvips.enums.Interpretation.B_W)
        .invert()
        for f in registrar.get_sorted_img_f_list()
    ]
    full_rez_overlap = create_overlap_img(warped_slide_list, blending="light")
    ...
    ome_obj = slide_io.create_ome_xml(
        shape_xyzct,
        bf_dtype,
        is_rgb=True,
    ...
    slide_io.save_ome_tiff(
        full_rez_overlap, dst_f=dst_f, ome_xml=ome_xml, compression="jpeg"
    )
...
def color_multichannel(
        ...
        multichannel_img = np.dstack(
            [
                exposure.rescale_intensity(
                    multichannel_img[..., i].astype(float),
                    in_range="image",
                    out_range=(0, 1),
                )
    ...
    is_srgb_01 = True
    if 1 < marker_colors.max() <= 255 and np.issubdtype(
        marker_colors.dtype, np.integer
    ):
...
def color_dxdy(dx, dy, c_range=DXDY_CRANGE, l_range=DXDY_LRANGE, cspace=DXDY_CSPACE):
    ...
    C = exposure.rescale_intensity(
        magnitude, in_range=(0, magnitude.max()), out_range=tuple(c_range)
    )
    H = np.arctan2(dy.T, dx.T)
    A, B = C * np.cos(H), C * np.sin(H)
    J = exposure.rescale_intensity(
        magnitude, in_range=(0, magnitude.max()), out_range=tuple(l_range)
    )
    ...
    displacement_rgb = (
        (255 * np.clip(rgb, 0, 1)).astype(np.uint8).reshape((*initial_shape, 3))
    )
...
def displacement_legend():
    ...
    displacement_legend = color_dxdy(
        dx, dy, DXDY_CRANGE, DXDY_LRANGE, cspace=DXDY_CSPACE
    )
...
def color_displacement_grid(
    ...
    warped_hcl = [None] * 3
    for i in range(3):
        warped_hcl[i] = transform.warp(
            grid_img[..., i], np.array([img_warp_r, img_warp_c])
        )
...
def draw_trimesh(shape_rc, tri_verts, tri_faces, thickness=2):
    ...
    for face in tri_faces:
        ...
        pt_order = np.argsort(
            [np.rad2deg(np.arctan2(xy[1] - cy, xy[0] - cx)) for xy in verts]
        )
        # draw points
        pts = verts[pt_order, :].reshape(-1, 1, 2).astype(int)
        tri_img = cv2.polylines(
            tri_img, [pts], True, 1, thickness, lineType=cv2.LINE_AA
        )

    return tri_img.astype(float)


def draw_displacement_vector_field(
    img, dxdy, spacing=25, brightness=0.7, cspace="OKLAB"
):
    start_r, start_c = np.meshgrid(
        np.arange(0, img.shape[0], spacing),
        np.arange(0, img.shape[1], spacing),
        indexing="ij",
    )
    end_r = start_r + dxdy[1][start_r, start_c]
    end_c = start_c + dxdy[0][start_r, start_c]

    start_xy = np.stack([start_c.reshape(-1), start_r.reshape(-1)]).T
    end_xy = np.floor(np.stack([end_c.reshape(-1), end_r.reshape(-1)]).T).astype(int)

    eps = np.finfo("float").eps
    sampled_dx = dxdy[0][start_r, start_c]
    sampled_dy = dxdy[1][start_r, start_c]
    angles = np.arctan2(sampled_dx, sampled_dy)
    mag = np.sqrt((sampled_dx**2 + sampled_dy**2 + eps))
    mag = exposure.rescale_intensity(mag, out_range=(0, 1))

    ab = colour.algebra.polar_to_cartesian(np.dstack([mag, angles]))
    jab = np.dstack([np.full_like(mag, brightness), ab])
    rgb = (
        (255 * np.clip(colour.convert(jab, cspace, "sRGB"), 0, 1))
        .astype(np.uint8)
        .reshape((-1, 3))
    )

    vector_img = img.copy()
    if vector_img.ndim == 2:
        vector_img = np.dstack(3 * [vector_img])

    for i in range(end_xy.shape[0]):
        arrow_start = start_xy[i]
        arrow_end = end_xy[i]
        arrow_rgb = rgb[i].tolist()

        vector_img = cv2.arrowedLine(
            img=vector_img,
            pt1=arrow_start,
            pt2=arrow_end,
            color=arrow_rgb,
...
def color_displacement_tri_grid(
    ...
    padded_dx = transform.warp(
        bk_dx, padding_T, output_shape=padded_shape, preserve_range=True
    )
    padded_dy = transform.warp(
        bk_dy, padding_T, output_shape=padded_shape, preserve_range=True
    )
    ...
    trimesh_img = transform.warp(
        trimesh_img, inv_T, output_shape=shape, preserve_range=True
    )
    colored_displacement = color_dxdy(
        bk_dx, bk_dy, c_range=c_range, l_range=l_range, cspace=cspace
    )
