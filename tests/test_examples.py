...
def check_for_no_transforms_in_ref(ref_slide, reference_img_fname):
    assert ref_slide.name == valtils.get_name(
        reference_img_fname
    ), "Reference image is not the same as specified image"
    ...
    try:
        ...
        assert np.all(
            tformer.scale == [1.0, 1.0]
        ), "Reference image has unexpected scaling"
        ...
        crop_bbox, _ = ref_slide.get_crop_xywh(
            ref_slide.crop, out_shape_rc=out_shape_rc
        )
        ...
        assert (
            np.dstack(ref_slide.bk_dxdy).min() == 0
            and np.dstack(ref_slide.bk_dxdy).max() == 0
        ), "Found unexpected non-rigid transforms"
        ...
        assert np.all(
            warped_origin == 0
        ), "reference image not warping points correctly"
...
def get_dirs():
    dst_parent_folder = (
        f"{sys.platform}_{sys.version_info.major}{sys.version_info.minor}"
    )
    try:
        ...
        split_idx = [
            i for i in range(len(dir_split)) if dir_split[i] == "valis_project"
        ][0]
        parent_dir = os.path.join(os.sep.join(dir_split[: split_idx + 1]), "valis")
        results_dst_dir = os.path.join(
            parent_dir, f"tests/{sys.version_info.major}{sys.version_info.minor}"
        )
...
def test_register_ihc(max_error=70):
    ...
    try:
        ...
        registered_slide_dst_dir = os.path.join(
            registrar.dst_dir, "registered_slides", registrar.name
        )
        registrar.warp_and_save_slides(
            dst_dir=registered_slide_dst_dir, Q=90, compression="jpeg"
        )
...
def test_register_cycif(max_error=3):
    ...
    try:
        ...
        dst_f = os.path.join(
            registrar.dst_dir, "registered_slides", f"{registrar.name}.ome.tiff"
        )
        ...
        assert [
            slide_obj.stack_idx for slide_obj in registrar.slide_dict.values()
        ] == list(range(registrar.size)), "Slides got sorted when `imgs_ordered=True`"

        sorted_img_list = (
            registrar.get_sorted_img_f_list()
        )  # Get images in the same order used for registration (may be different than order in directory)
        expected_channel_order = list(
            chain.from_iterable([channel_name_dict[str(f)] for f in sorted_img_list])
        )
        if drop_duplicates:
            cnames_df = pd.DataFrame(expected_channel_order, columns=["cname"])
            expected_channel_order = list(
                cnames_df.drop_duplicates(keep="first")["cname"]
            )

        saved_ome_xml = ome_types.from_xml(ome_xml)
        saved_channel_names = [x.name for x in saved_ome_xml.images[0].pixels.channels]

        assert np.all(expected_channel_order == saved_channel_names), (
            f"Channels not saved in correct order.\n"
