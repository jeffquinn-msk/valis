...
from ome_types._autogenerated.ome_2016_06 import Channel, TiffData, Plane, Pixels, UnitsLength, Image
...
def check_and_correct_orientation(
    ...
    max_useful = min(
        ...
        moving_img.width, moving_img.height,
    )
    effective_size = min(downsample_size, max_useful)
    if effective_size != downsample_size:
        print(f"[orientation] requested size {downsample_size} clamped to "
    ...
    if match.k == 0 and not match.mirror:
        ...
        print(f"  >> applying {orientation_check.describe(match)} to moving image "
              "before registration.")
    print("=========================\n")
...
def warp_new_slide(
    ...
    warped_img = src_slide_obj.warp_img_from_to(
        new_img,
        to_slide_obj=dst_slide_obj
    )
...
def write_ome_tiff(
    ...
    for img in images:
        if img.width != first_width or img.height != first_height:
            raise ValueError(f"mismatched image size:"
