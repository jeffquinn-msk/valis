...
sys.path.append(
    "/Users/gatenbcd/Dropbox/Documents/image_processing/valis_project/valis"
)
import time
import torch
import numpy as np
...
def get_dirs():
    ...
    if not in_container:
        ...
        results_dst_dir = os.path.join(
            parent_dir, f"valis/tests/{sys.version_info.major}{sys.version_info.minor}"
        )
    else:
...
dedode_matcher_obj = feature_matcher.LightGlueMatcher(
    feature_detectors.DeDoDeFD(rgb=False)
)
...
default_reg_idx = [
    ...
    if not isinstance(matcher_list[i], feature_matcher.LightGlueMatcher)
][0]
...
for slide_obj in slide_list:
    ...
    plt.savefig(
        os.path.join(results_dst_dir, f"{moving_slide_name}_to_{fixed_slide_name}.png")
    )
    plt.close()
