...
def match_desc_and_kp(
    ...
    if filtering_kwargs is None:
        if filter_method != RANSAC_NAME:
            print(
                Warning(
                    f"filtering_kwargs not provided for {filter_method} match filtering. Will use RANSAC instead"
                )
            )
...
class SuperPointAndGlue(Matcher):
    ...
    def match_images(
        ...
        if img1 is not None and img2 is not None:
            return self._match_images(
                img1, img2, additional_filtering_kwargs=additional_filtering_kwargs
            )
        else:
            return self._match_kp(
                desc1=desc1,
                kp1_xy=kp1_xy,
                desc2=desc2,
                kp2_xy=kp2_xy,
                additional_filtering_kwargs=additional_filtering_kwargs,
            )
    ...
    def _match_images(
        ...
        **kwargs,
        ...
        if (
            n_sg_matches < 4
            or self.match_filter_method.lower() == SUPERGLUE_FILTER_NAME
        ):
            ...
            if n_sg_matches == 0:
                ...
                match_distances = metrics.pairwise_distances(
                    desc1, desc2, metric=metric
                )
            ...
            (
                ...
                desc1, kp1_pos_xy, desc2, kp2_pos_xy, additional_filtering_kwargs
            )
    ...
    def _match_images(
        ...
        if (
            ...
            (
                ...
                sg_desc1, kp1_pos_xy, sg_desc2, kp2_pos_xy, additional_filtering_kwargs
            )
