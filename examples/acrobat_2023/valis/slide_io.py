...
class SlideReader(object):
    ...
    def get_channel_index(self, channel):
        ...
        if isinstance(channel, int):
            ...
            matching_channels = [
                i
                for i in range(self.metadata.n_channels)
                if re.search(channel.lower(), self.metadata.channel_names[i].lower())
                is not None
            ]

            if len(matching_channels) == 0:
                msg = (
                    f"Cannot find channel '{channel}' in {self.src_f}. Using channel 0"
                )
                valtils.print_warning(msg)
                matching_channel_idx = 0

            elif len(matching_channels) > 1:
                all_matching_channels = ", ".join(
                    [f"'{self.metadata.channel_names[i]}'" for i in matching_channels]
                )
...
class VipsSlideReader(SlideReader):
    ...
    def create_metadata(self):
        ...
        if f_extension in BF_READABLE_FORMATS:
            ...
            self.is_ome = (
                re.search("ome-tiff", bf_reader.metadata.server.lower()) is not None
            )
        ...
        if f_extension in BF_READABLE_FORMATS:
            ...
            slide_meta.pixel_physical_size_xyu = (
                bf_reader.metadata.pixel_physical_size_xyu
            )
    ...
    def slide2vips(self, level, xywh=None, *args, **kwargs):
        ...
        if self.use_openslide:
            vips_slide = pyvips.Image.new_from_file(
                self.src_f, level=level, access="random"
            )[0:3]
    ...
    def _get_slide_dimensions_openslide(self, vips_img):
        ...
        slide_dims = np.array(
            [
                [
                    eval(vips_img.get(f"openslide.level[{i}].width")),
...
class CziJpgxrReader(SlideReader):
    ...
    def slide2vips(self, level, xywh=None, *args, **kwargs):
        ...
        if self.is_bgr:
            ...
            print(
                "currently only support RGB images when the CZI images are compressed using JPGXR"
            )
            vips_img = None
    ...
    def _get_zoom_levels(self, scene=0):
        ...
        img_dict = self._get_img_meta_dict()
        pyramid_info = img_dict["Dimensions"]["S"]["Scenes"]["Scene"][scene][
            "PyramidInfo"
        ]
    ...
    def _read_rgb(self, level=0, xywh=None, *args, **kwargs):
        ...
        vips_img = (
            pyvips.Image.black(*out_shape_wh, bands=self.metadata.n_channels)
            + bg_rgba[0:3]
        )
...
def get_slide_reader(src_f, series=None):
    ...
    if can_only_use_openslide and not can_use_openslide:
        msg = (
            f"file {os.path.split(src_f)[1]} can only be read by OpenSlide, "
    ...
    if (
        not can_use_openslide
        and not can_use_bf
        and not can_use_skimage
        and not can_use_pyvips
    ):
...
def create_ome_xml(
    ...
    if is_rgb:
        ...
        default_colormap = {
            channel_names[i]: tuple(default_colors[i]) for i in range(c)
        }
        ...
        channels = [
            create_channel(i, name=channel_names[i], color=colormap[channel_names[i]])
            for i in range(c)
        ]
        ...
        try:
            ...
            if colormap is None:
                ...
                if len(all_rgb) == 1 and nc > 1:
                    ...
                    colormap = {
                        img.pixels.channels[i].name: tuple(default_colors[i])
                        for i in range(nc)
                    }
...
def convert_to_ome_tiff(
    ...
    out_xyczt = get_shape_xyzct(
        (vips_img.width, vips_img.height), slide_meta.n_channels
    )
    ...
    save_ome_tiff(
        vips_img, dst_f, ome_xml_str, tile_wh=tile_wh, compression=compression
    )
